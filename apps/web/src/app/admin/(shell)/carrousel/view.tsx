...
export function HeroView() {
  const { data, error, loading, reload } = useAdminData<HeroSlideImage[]>(() =>
    adminFetch<HeroSlideImage[]>('/hero-slides'),
  );

  if (loading) return <AdminLoading />;
  if (error || !data) return <AdminError message={error} onRetry={reload} />;

  const bySlide = new Map(data.map((override) => [override.slideId, override]));
